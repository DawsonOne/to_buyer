    ...
    export default {
        ...
        methods:{
            pay(){
                ...
                const _this = this
                axios.put('http://localhost:8383/order-service/buyer/order/pay/'+JSON.parse(window.localStorage.getItem('access-user')).openid+'/'+this.$route.query.orderId).then(function (resp) {
                    let instance = Toast('支付成功');
                    setTimeout(() => {
                        instance.close();
